...
{% set empty_bracket = "\n(\n)"%}
{% endif %}
CREATE {% if data.relpersistence %}UNLOGGED {% endif %}TABLE{% if add_not_exists_clause %} IF NOT EXISTS{% endif %} {{qtIdent(data.schema, data.name)}}{{empty_bracket}}
{% if data.typname %}
    OF {{ data.typname }}
{% endif %}
...
{{CONSTRAINTS.PRIMARY_KEY(conn, data.primary_key[0])}}
    {% if data.columns|length > 0 %}

    {% if data.name %}CONSTRAINT {{qtIdent(data.name)}} {% endif %}PRIMARY KEY ({% for c in data.columns%}
{% if loop.index != 1 %}, {% endif %}{{qtIdent(c.column)}}{% endfor %}){% if data.include|length > 0 %}

        INCLUDE({% for col in data.include %}{% if loop.index != 1 %}, {% endif %}{{qtIdent(col)}}{% endfor %}){% endif %}
{% if data.fillfactor %}

        WITH (FILLFACTOR={{data.fillfactor}}){% endif %}
{% if data.spcname and data.spcname != "pg_default" %}

        USING INDEX TABLESPACE {{ qtIdent(data.spcname) }}{% endif %}
{% if data.condeferrable %}

        DEFERRABLE{% if data.condeferred %} INITIALLY DEFERRED{% endif%}{% endif%}
{% endif %}
    {% endif %}{% if data.unique_constraint|length > 0 %}{% if data.columns|length > 0 or data.primary_key|length > 0 %},{% endif %}
...
TABLESPACE {{ qtIdent(data.spcname) }};
{% else %}
);
{% endif %}
{### Alter SQL for Owner ###}
...
ALTER TABLE IF EXISTS {{qtIdent(data.schema, data.name)}}
    OWNER to {{qtIdent(data.relowner)}};
{% endif %}
...
{### SQL for COMMENT ###}
{% if data.description %}
COMMENT ON TABLE {{qtIdent(data.schema, data.name)}}
    IS {{qtLiteral(data.description)}};
{% endif %}
{#===========================================#}
...
ALTER TABLE IF EXISTS {{qtIdent(data.schema, data.name)}}
    ALTER COLUMN {{qtTypeIdent(c.name)}} SET STATISTICS {{c.attstattarget}};

{% endif %}
{###  Alter column storage value ###}
{% if c.attstorage is defined and c.attstorage != c.defaultstorage %}
ALTER TABLE IF EXISTS {{qtIdent(data.schema, data.name)}}
    ALTER COLUMN {{qtTypeIdent(c.name)}} SET STORAGE {%if c.attstorage == 'p' %}
